...
| [ADR-022](ADR-022-production-backup-dr-strategy.md) | Production Backup & Disaster Recovery Strategy | **ACCEPTED (2026-09-08)** — a 2026-09-07 decision-authority review had found a PRODUCT GOVERNANCE GAP (no repository document named who could set RPO/RTO/PITR-retention or accept this ADR); the Product Owner then explicitly established themselves as that authority and supplied RPO=1 hour, RTO=1–4 hours, PITR retention=7 days, plus a qualitative scale-up review trigger — closing the gap by direct decision rather than by discovering pre-existing authority | — | — | Database / Operations / Cost | No — completes, rather than resolves, a gap ADR-006 left open (it named "regular backups" as a security property but never selected a specific backup/PITR strategy); target production architecture is Hybrid + PITR (Pro managed daily backups + PITR at the approved 7-day retention, + independent logical backups), both required unconditionally by SDD Ch.12 §12.7/Ch.16 §16.7; acceptance is a decision-authority/architecture-design milestone only — no production Supabase project, PITR configuration, or backup automation has been created; **DigiHostel's current implementation remains intentionally Free-tier-first** (2026-09-08 addendum) — this ADR defines the future Pro+PITR scale-up target, not the current environment; see ADR-022's "Not Yet Implemented" and "Addendum: Free-Tier-First Implementation Note" sections |
