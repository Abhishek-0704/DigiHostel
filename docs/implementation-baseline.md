...
**Update (2026-09-24):** a real production Supabase project now exists (`asphlfoikqyaeslmhrah`) and is also, by explicit decision, currently on the Free tier with no PITR/managed backups — see [`ADR-026`](adr/ADR-026-interim-production-recovery-risk-acceptance.md) (ACCEPTED, partially superseding ADR-022), which extends the deliberate-deferral posture described above to production specifically, now that production genuinely exists rather than remaining a future prerequisite. ADR-022's target architecture and approved DR values are unchanged; only the "reached before/at go-live" expectation is superseded.
