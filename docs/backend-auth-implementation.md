...
## Future Device Re-Attestation / Biometric Gates (`lib/auth/security-gates.ts`)

`DeviceAttestationGate` and `BiometricFreshnessGate` are typed interfaces only, and remain so. The only production-registered implementations, `NotImplementedAttestationGate`/`NotImplementedBiometricFreshnessGate`, throw rather than silently succeeding — no route calls them, and no code claims a result via these two interfaces exists. `requireActiveTrustedDevice` (device-trust **validation** — is this device currently marked trusted, per real `trusted_devices.revoked_at` data) is real today.

**Registration-time device attestation (ADR-003) is now real** (implemented after this document was originally written — see the ADR-003 implementation report, `docs/adr/ADR-003-implementation-report.md`): a device proves itself via Google Play Integrity, server-verified, before it is ever marked trusted (`apps/api/src/domain/device/` — `AttestationVerifier`, `PlayIntegrityVerifier`, `DeviceRegistrationService` — a separate module from `security-gates.ts`, since `DeviceAttestationGate`'s `checkAttestation(trustedDeviceId)` shape presupposes a device that already exists). `DeviceAttestationGate` here remains for a distinct, still-unbuilt capability: re-checking an *already-trusted* device's attestation after the fact. Biometric freshness remains genuinely unimplemented in the cryptographic sense (see `AssertionPresenceBiometricFreshnessGate`'s own doc comment for its current, honest, non-cryptographic placeholder used by the leave-approval workflow). Future work wires a real re-attestation provider behind `DeviceAttestationGate` without changing route authorization structure.
