...
**`/api/v1/profile` (Phase 7, Prompt 17 — Administrative Profile & Personal Preferences Center)** — self-scoped only, any staff role (`reception_warden`/`hostel_admin`/`library_incharge`/`super_admin`), no AAL2 requirement (personal, non-privileged preference changes carry no privilege-escalation risk), not part of the SDD's original endpoint-family list at all. Every route resolves exclusively to the caller's own server-resolved staff id — never a client-supplied one, and never any other staff member's:

- `GET /api/v1/profile` — the caller's own identity (read-only: role/hostel/status remain owned by Identity & Access Administration) plus personal preferences, transparently creating a default preferences row on first access.
- `PATCH /api/v1/profile` — a partial update. `fullName` writes to the caller's own `staff.full_name` (the pre-existing self-update RLS authority); every other field writes to a new `staff_preferences` table (contact info, theme/density/font-scale/date-format, reduced-motion/high-contrast, default landing page, notification category preferences, dashboard preferences, personal shortcuts). Role/permissions/hostel assignment/account status are not declared on this endpoint's schema at all — an attempt to send one is rejected `400`, never silently applied. A mandatory safety notification category (`emergency_alert`/`health_alert`) cannot be disabled.

`GET /api/v1/audit` also gained one additive, optional `actorId` filter (Personal Activity — applied after the existing hostel-scope check, zero behavior change for every existing caller that omits it), and `POST /auth/staff/audit-events` gained one new reportable event, `sessions_signed_out_others`. See `apps/reception-dashboard/docs/admin-profile-preferences.md` for the full architecture record, the `staff_preferences` RLS model (deliberately no super_admin bypass), and the complete list of deferred/unavailable capabilities.

**`/api/v1/monitoring` (Phase 7, Prompt 18 — Enterprise Operations Monitoring Center)** — `super_admin`-only (AAL2-required, reusing the existing `system:view` permission, granted to no other role since Prompt 3), not part of the SDD's original endpoint-family list at all. A pure read-model layer over genuinely-measured infrastructure signals and read-only reuse of already-certified domain services (Analytics/Emergency/Health) — no new persistent table, no mutation of any operational data:

- `GET /api/v1/monitoring/overview` — aggregated platform/application/operational/security health in one response (database connectivity, Supabase Auth Admin API reachability, `supabase_realtime` publication integrity, emergency/health/leave/presence counts, recent MFA-failure/suspended-account/administrative-change counts, deployment version/environment, and live-derived alerts). Every signal is either directly measured or reused from an existing service — never fabricated; an unmeasurable dependency reports `unknown`/`unavailable`, never a fabricated `healthy`.
- `GET /api/v1/monitoring/diagnostics` — the fixed, server-owned diagnostic catalog (4 checks).
- `POST /api/v1/monitoring/diagnostics/{diagnosticId}/run` — executes one allow-listed, read-only, timeout-bounded diagnostic; an unrecognized id is rejected `404`, never executed. Recorded as one `monitoring.diagnostic_run` row in the existing `audit_logs` trail — no new audit mechanism.

See `apps/reception-dashboard/docs/monitoring-center.md` for the full Platform Health Model, aggregation rules, diagnostic allow-list, alert-derivation strategy, and the complete list of deferred/unavailable metrics with reasons.

No other endpoint family listed above (`/auth/*` beyond what's implemented, `/library/*`, `/notifications`) is implemented yet — do not assume they exist merely because the SDD specifies them (`docs/current-state.md`). This document itself remains a known-stale, partial record of the full API surface (many staff-facing endpoint families built since — Student Operations, Movement, Emergency, Health — are not listed above at all; see `docs/current-state.md` for the authoritative, up-to-date implementation history) — out of scope to fully rewrite here.
