...
- **Reception/Library/Admin web dashboards**: separate React web client(s), not part of this ADR set's mobile decisions but consuming the same backend/API and RBAC model. The Reception Dashboard's web framework is now decided (Vite+React+TS SPA, ADR-023) and scaffolded (`apps/reception-dashboard`, Prompt 0.2) — Library and Admin dashboards remain undecided/unscaffolded, each pending its own future ADR when that work begins.
