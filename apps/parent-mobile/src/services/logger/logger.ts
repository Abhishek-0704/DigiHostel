/**
 * Logging abstraction (Prompt 2 foundation). A thin wrapper over `console`
 * today — its value is the single choke point every future feature logs
 * through, so swapping in a real observability/crash-reporting SDK later is
 * a one-file change, not a repo-wide find-and-replace. No such SDK is
 * installed in this pass — not justified by any current requirement.
 *
