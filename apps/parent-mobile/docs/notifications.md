...
| Realtime Postgres Changes on `notifications` | **PARTIALLY IMPLEMENTED** | ADR-009 accepts this architecture and the Realtime *service* is enabled (`supabase/config.toml`), but no migration adds `notifications` to a realtime publication (no `PUBLICATION` statement exists anywhere in `supabase/migrations/`). The client subscription built in this prompt is real and ADR-009-correct; it will connect but is not confirmed to receive change events until that migration lands. See §7. |
...
**Capability caveat (see §1):** this subscription is real and correct per ADR-009, but no migration in this repository adds `notifications` to a realtime publication. It will connect (`"connected"` status achievable) but change-event delivery is not confirmed working end-to-end in this environment (no local Supabase/Docker stack was running during this prompt — see §14). Pull-to-refresh remains the reliable path to see new notifications regardless of realtime status.
